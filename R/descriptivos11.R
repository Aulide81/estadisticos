...
bbdd <- data.frame(Valor=attr(x, "val.lab"))
return(bbdd)
}
...
cross<-function(x,y,w,cells="count",dec=1,orden,ancho=12,Totrow=T,Totcol=T,selectrow,selectcol){
  ...
  absolutos<-absolutos[!(rowSums(absolutos)==0),!colSums(absolutos)==0,drop=F]
...
etiquetasx<-sort(c(attr(x,"val.lab"),indices[etiquetasx==F]))
...
etiquetasy<-c("",paste(indices,etiquetasy),"Total")   
  ...
  attr(tabla,"pct")<-pct
  attr(tabla,"resumen")<-paste("Number of Missing Observations:",round(sum(w,na.rm=T),0)-absolutos[nrow(absolutos),ncol(absolutos)])
  attr(tabla,"title")<-paste(names(as.list(attr(x,"var.lab"))),attr(x,"var.lab"),"by",names(as.list(attr(y,"var.lab"))),attr(y,"var.lab"))
...
k.means<-function(x,centers,w,iter=10,initial,seed){
  ...
  if (missing(initial)){
    ...
    if (!is.matrix(initial)) stop("Los valores iniciales han de ser una matrix")
    if (ncol(initial)!=ncol(x)) stop("El numero de columnas de initial es diferente a de x")
    if (nrow(initial)!=centers) stop("El numero de filas de initial tiene que ser igual al numero de clusters (centers)")
    centroides<-initial  
  }

  
  centroidesinit<-centroides
  if(ncol(x)==1){ dim(centroidesinit)<-c(centers,ncol(x))
  colnames(centroidesinit)<-colnames(x)
  centroides<-centroidesinit
  }
  
  mdist<-apply(x,1,function(x)dist(rbind(x,centroides)))
  
  
  mdist<-t(mdist)
  mdist<-mdist[,1:centers]
  
    
  grupofinal<-as.integer(apply(mdist,1,which.min))
  ...
  while (condicion==F){
    ...
    if (contador==iter){
      warning("Se han alcanzado el numero maximo de iteraciones:",iter,call.=F)
      break
    } 
  }
  

  centroides<-apply(x,2,.meansby,y=grupofinal,w=w)
  etiquetas<-NULL
  ...
  rownames(centroides)<-rownames(c(1:centers),do.NULL=F,prefix="Grupo.")  
  centroides<-t(centroides)
  
  colnames(centroidesinit)<-paste(names(x),etiquetas)
  rownames(centroidesinit)<-rownames(c(1:centers),do.NULL=F,prefix="Grupo.")  
  centroidesinit<-t(centroidesinit)
  
  sct<-apply(x,2,.sct,w=w)
  scd<-apply(x,2,.scd,y=grupofinal,w=w)
  sce<-apply(x,2,.sce,y=grupofinal,w=w)
  
  matriz<-cbind(MQ_Grupos=colSums(sce)/(centers-1)
                ,gl=(centers-1)
                ,MQ_Error=colSums(scd)/apply(x,2,function(j)sum(w[complete.cases(j)]))
                ,gl=apply(x,2,function(j)sum(w[complete.cases(j)]))-centers)
  
  matriz<-cbind(matriz,F=matriz[,1]/matriz[,3])
  rownames(matriz)<-paste(rownames(matriz),etiquetas) 
    
  dim(grupofinal)<-c(length(grupofinal),1)
  rownames(grupofinal)<-rownames(x)
  
  resultados<-list(Centros_Iniciales=centroidesinit,Grupos=grupofinal,Centros_Finales=centroides,
                   Iterations=contador,Variacion_Total=sct,Variacion_Intra_Grupo=scd,Variacion_Entre_Grupo=sce,Anova=matriz)
  
  print(resultados[1])
  print(resultados[3])
  
  cat("Suma de Variacion Intra Grupos por clusters\n")
  print(rowSums(scd))
  cat("\n(Variacion_Entre_Grupo/Variacion_Total =",round(sum(sce)/sum(sct)*100,2),"%)\n\n")
  print(resultados[8])
...
addvallab<-gtools::defmacro(df,var,val,
                            expr={
                              ...
                              if (length(a[a%in%val])>0) names(a)[a[a%in%val]]<-names(val)[val%in%a[a%in%val]]
                              attr(df$var,"val.lab")<-sort(c(a,val[!val%in%a[a%in%val]]))
                            })
sumar<-function(...){
  x<-cbind(...)
